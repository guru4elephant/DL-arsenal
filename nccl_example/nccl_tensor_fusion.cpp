...
{
    if (argc != 5) {
        LOGERR("%s size matmul_num send_buf_size fuse", argv[0]);
        exit(-1);
    }
    int size = atoi(argv[1]); // 1024
    int mat_num = atoi(argv[2]); // 10
    int send_size = atoi(argv[3]); // 1024 * 1024 * 10
    int fuse_int = atoi(argv[4]);
    ...
    if (fuse == true) {
        ...
        for (int i = 0; i < 1000; ++i) {
            for (int j = 0; j < mat_num; ++j) {
                NCCLCHECK(ncclAllReduce((const void *)(send_buff + j * bucket), 
                                        (void *)(recv_buff + j * bucket), 
                                        bucket,
                                        ncclFloat, ncclSum, comm, s));
                CUDACHECK(cudaStreamSynchronize(s));
            }
