...
def multi_download(client,
    ...
    need_downloads = [x for x in all_need_download if x.startswith("part-")]
    need_download = need_downloads[trainer_id::trainers]
    _logger.info("Get {} files From all {} files need to be download from {}".
                 format(len(need_download), len(need_downloads), hdfs_path))
...
if __name__ == "__main__":
    ...
    hadoop_home = "/home/tangw/client/hadoop-client/hadoop/"
    configs = {}
    configs["fs.default.name"] = "hdfs://yq01-heng-hdfs.dmop.baidu.com:54310"
    configs["hadoop.job.ugi"] = "nlp,hello123"
    client = HDFSClient(hadoop_home, configs)
    multi_upload(client, "/app/ssg/nlp/sc/heqiaozhi/cpu-benchmark-temp/model/tangwei12", "/home/tangw/src/test/data1")
    sys.exit(0)

    client.ls("/app/ssg/nlp/sc/heqiaozhi/cpu-benchmark-temp/train-25")
    files = client.lsr("/app/ssg/nlp/sc/heqiaozhi/cpu-benchmark-temp/model")

    downloads = multi_download(client, "/app/ssg/nlp/sc/heqiaozhi/cpu-benchmark-temp/model", "/home/tangw/src/test/data1", 0, 1, multi_processes=5)
    for d in downloads:
        print d
