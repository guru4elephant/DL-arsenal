...
=======================================================\033[0m'''
    ...
    def run_from_files(self, 
        ...
        processes.close()
        processes.join()
        infos = [result.get() for result in info_result]
        proto_info = self._combine_infos(infos)
        with open(os.path.join(self._output_dir, self._proto_filename), "w") as f:
    ...
    def _subprocess(self, file_queue, remaining_queue, output_index, lock):
        ...
        while not file_queue.empty():
            ...
            with open(filename, 'r') as f:
                for line in f:
                    line_iter = self.process(line)
                    for read_line in line_iter():
                        if read_line == None:
                            continue
                        buffer.append(self._gen_str(read_line))
                        if len(buffer) == self._line_limit:
                            with open(self._get_output_filename(output_index, lock), "w") as wf:
                                for x in buffer:
                                    wf.write(x)
                            # put current file to a ready file list
                            buffer = []
        if buffer:
            for x in buffer:
...
def subprocess_wrapper(instance, file_queue, remaining_queue, output_index, lock):
    '''
